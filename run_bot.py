...
def main():
    ...
    # Leggi i locali dal database
    with app.app_context():
        # Se è specificato un LOCALE_ID, esegui solo quello
        if locale_id_richiesto:
            try:
                ...
                if not locale.attivo:
                    print(f"⚠️ ATTENZIONE: Locale {locale.nome} non è attivo!")
                    print("   Esecuzione annullata")
                    sys.exit(0)

                print(f"🚀 ESECUZIONE MANUALE RICHIESTA")
                print(f"   Locale: {locale.nome}")
                print(f"   Modalità: Istantanea (non programmata)\n")

                locali_da_processare = [locale]

            except ValueError:
                print(f"❌ ERRORE: LOCALE_ID non valido: {locale_id_richiesto}")
                sys.exit(1)
        else:
            # Esecuzione programmata: leggi tutti i locali attivi
